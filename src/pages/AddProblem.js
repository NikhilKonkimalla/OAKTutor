...
const AddProblem = () => {
    ...
    const handleSaveProblem = () => {
        ...
        try {
            const problemId = problemData.id || generateId();
            const newProblem = {
                ...problemData,
                id: problemId,
                lesson: lessonID,
                courseName: 'Custom Course', // This should be dynamically determined
                variabilization: {},
            ...
            localStorage.setItem('customProblems', JSON.stringify(savedProblems));
            
            console.log('Problem saved successfully:', newProblem);
            alert(`Problem "${problemData.title}" saved successfully!`);
    ...
    return (
        <div className={classes.root}>
            <AppBar position="static" className={classes.appBar}>
                <Toolbar>
                    <IconButton 
                        ...
                        onClick={() => history.push(`/lessons/${lessonID}`)}
                    >
                        <ArrowBackIcon />
                    </IconButton>
            ...
            <div className={classes.container}>
                    ...
                    <CardContent>
                        ...
                        <Grid container spacing={2}>
                            <Grid item xs={12} sm={6}>
                                <TextField
                                    fullWidth
                                    label="Problem ID (optional - will auto-generate if empty)"
                                    value={problemData.id}
                                    onChange={(e) => handleProblemChange('id', e?.target?.value || '')}
                            ...
                            <Grid item xs={12} sm={6}>
                                <Button
                                    variant="outlined"
                                    onClick={() => handleProblemChange('id', generateId())}
                ...
                {customProblems.filter(problem => problem.lesson === lessonID).length > 0 && (
                    <Card className={classes.card} style={{ marginTop: '32px' }}>
                        <CardContent>
                            <Typography variant="h6" className={classes.sectionTitle}>
                                Manage Existing Problems
                            </Typography>
                            {customProblems
                                .filter(problem => problem.lesson === lessonID)
                                .map((problem, index) => (
                                    <Box key={problem.id} style={{
                                        backgroundColor: '#f8f9fa',
                                        border: '1px solid #e9ecef',
                                        borderRadius: '6px',
                                        padding: '16px',
                                        ...
                                        <Box>
                                            ...
                                            <Typography variant="body2" color="textSecondary">
                                                {problem.steps ? problem.steps.length : 0} steps
                                            </Typography>
                                            <Typography variant="body2" color="textSecondary">
                                                ID: {problem.id}
                                            </Typography>
                                        ...
                                        <Box>
                                            <Button
                                                ...
                                                size="small"
                                                startIcon={<EditIcon />}
                                                onClick={() => history.push(`/lessons/${lessonID}/edit-problem/${problem.id}`)}
